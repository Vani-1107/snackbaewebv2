  ...
  return (
    <div className='w-full h-fit '>
      ...
      <div className='w-full h-[calc(100vh-70px)] relative '>
        <img src={blogHero} alt='blogHero' className='w-full h-full object-cover absolute z-[-1] brightness-75' />
      ...
      <div className='w-full h-fit lg:p-[3rem] flex lg:flex-row flex-col items-center justify-center py-[3rem]'>
        <img src={blogsecondhero} alt='blogsecondhero' className='w-[90%] sm:w-[80%] lg:w-[50%] aspect-auto' />
        <div className='bg-[#ffffff] rounded-lg w-[80%] sm:w-[60%] lg:w-[40%] h-fit py-[3rem] px-[2rem] shadow-lg relative top-[-50px] lg:top-0 lg:left-[-5%] '>
          <p className=' font-inter text-[.9rem] text-[#232536] font-[400] leading-[1.4rem]'>At Snackbae, we understand the importance of staying ahead in today's dynamic business landscape. Our blogs serve as a valuable resource for merchants seeking to navigate challenges, capitalize on opportunities, and achieve sustainable growth.</p>
          <p className=' font-inter text-[.8rem] text-[#6D6E76] font-[400] leading-[1.4rem] mt-[.5rem]'>By sharing insights, strategies, and success stories, we empower merchants like you to thrive in an ever-evolving market. Stay informed, inspired, and equipped for success with our blog – your trusted companion on the journey to merchant excellence.</p>
        </div>
      </div>
      ...
      <div id='blogs' className='w-full h-fit'>
        {blogs.map((blog) => (
          <div className='w-[90%] mx-auto h-fit p-[2rem] flex md:flex-row flex-col items-center md:items-start justify-center gap-[2rem] ' key={blog.id}>
            ...
            <img src={blog.image} alt='image1' className='w-[360px] h-[240px] object-contain' />
            </Link>
            <div className='md:w-[50%] py-[1rem]'>
              <p className='text-[#232536] font-Sen font-[700] text-[2.4rem] leading-[3rem] tracking-tight'>{blog.title}</p>
